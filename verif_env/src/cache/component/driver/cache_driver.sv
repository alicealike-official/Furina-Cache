class cache_driver extends uvm_driver #(cache_transaction);
    ...
    function new(string name = "cache_driver",
    ...
    extern task drive_transaction(cache_transaction tr);

endclass

function void cache_driver::build_phase(uvm_phase phase);
...
task cache_driver::drive_transaction(cache_transaction tr);
    wait(cache_vif.rst_n);
    $display(tr.convert2string());
    cache_vif.cpu_req <= tr.cpu_cache_tr.cpu_req;
    cache_vif.cpu_wr_en <= tr.cpu_cache_tr.cpu_wr_en;
    cache_vif.cpu_req_addr <= tr.cpu_cache_tr.cpu_req_addr;
    cache_vif.cpu_wdata <= tr.cpu_cache_tr.cpu_wdata;
